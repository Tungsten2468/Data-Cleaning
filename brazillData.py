...
queryOrderValue = '''SELECT
                    ROUND(AVG(i.total_payment), 2) as c
                    FROM organized_data i'''
synqueryOrderValue = '''SELECT
                    ROUND(AVG(i.total_payment), 2) as c
...
# Example usage assuming item1 is the label index (0) and item2 is the number index (1):
# piegraph("SELECT method, amount FROM transactions", var1, var2, 0, 1)
...
'''
barGraph(queryOrderValue,synqueryOrderValue, "Different charts", "Order Value", 0, 1)
